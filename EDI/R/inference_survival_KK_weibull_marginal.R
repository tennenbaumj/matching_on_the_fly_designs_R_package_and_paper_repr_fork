...
#' dependence using \code{survival::survreg(..., cluster = ..., robust = TRUE)}.
#'
#' @examples
#' \donttest{
...
InferenceSurvivalKKWeibullMarginal = R6::R6Class("InferenceSurvivalKKWeibullMarginal",
	...
	private = as.list(modifyList(as.list(InferenceMixinKKPassThrough$private), list(
		...
		fit_weibull_marginal_survreg = function(X_fit, robust = TRUE, cluster_ids = NULL){
			...
			list(beta_T = beta_T, se_T = se_T, survreg_fit = fit)
		},
		shared = function(estimate_only = FALSE){
			...
			X_full = if (is.null(X_cov) || ncol(as.matrix(X_cov)) == 0L) {
				...
				cbind(treatment = private$w, as.matrix(X_cov))
			}
			...
			attempt = private$fit_with_hardened_qr_column_dropping(
				...
				fit_fun = function(X_fit){
					private$fit_weibull_marginal_survreg(X_fit, robust = !estimate_only, cluster_ids = cluster_ids)
				},
			...
			private$cached_mod = fit$survreg_fit
		...
		compute_treatment_estimate_during_randomization_inference = function(estimate_only = TRUE){
			...
			fit = private$fit_weibull_marginal_survreg(X_fit, robust = FALSE, cluster_ids = NULL)
